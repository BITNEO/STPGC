...
# Take only the first graph data
...
def draw_graph(edges,i): 
    # Remove rows of collapsed_adj_matrix
    # filtered_adj_matrix = collapsed_M[~deleted_node, :]
...
class nodes:
    def __init__(self, index):  # Initialize with x, y coordinates and length l of left endpoint
        self.index = index
        self.vanished = 0  # Number of overlapping points at this node
        self.edgenode = 0  # Number of edges at this node
        self.nodes = []  # List of overlapping points at this node
        self.ed_van = []  # Features obtained by removing edges at this node
        self.train_node = False  # Whether it is a training node
        self.remain = False  # Whether it needs to be retained
        self.recast = 0  # Whether it is a training node
        self.label = -1
       

    def __lt__(self, other):  # Overload operator "<" for heap optimization
    ...
    def __gt__(self, other):  # Overload operator ">" for heap optimization
        if self.edgenode > other.edgenode:
            return True
        else:
            if self.edgenode == other.edgenode and self.recast > other.recast:
    ...
    def __eq__(self, other):  # Overload operator "=" for heap optimization
        if self.edgenode == other.edgenode and self.recast == other.recast and self.vanished == other.vanished:
            return True
        else:
            return False
...
def find_component(data):
    ...
    for i in range(len(data['edge_index'][0])):
        ...
        #     print("???")
        # n[int(data['edge_index'][0, i])].edgenode += 1              # Note: For arxiv dataset, should modify to add edges to both endpoints
        # n[int(data['edge_index'][1, i])].edgenode += 1              # Note: For arxiv dataset, should modify to add edges to both endpoints
    # print(graph.nodes())
    # print(graph.edges())
    cnt_cluster_node = 0
...
# If want to convert to NumPy array

# b1,b2 = compute_betti_numbers(adj)
# print(f"Betti 1 (number of cycles): {b1}")
# print(f"Betti 2 (number of cavities): {b2}")
...
def sorted_list_intersection_with_deleted( list1, list2):
    ...
    # Use two pointers to traverse two lists
    while i < n and j < m:
        # Skip elements marked as deleted in list1
        
        # If one of the lists is traversed, end loop
...
class CoreAlgorithm:
    ...
    def approximate_persistence(self):
        ...
        edges = self.get_all_edges()
        # Sort edges in descending order according to the value of filtration_value_dict
    ...
    def delete_heterophlic_edge_inner(self,obj):
        ...
        for e in self.remain_edges:
            ...
            if  self.label[e[0]] != self.label[e[1]] and  self.label[e[0]] != -1 and self.label[e[1]] != -1: 
    ...
    def sorted_list_intersection_with_deleted(self, list1, list2):
        ...
        while i < n and j < m:
            ...
            # Skip elements marked as deleted in list2
            while j < m and self.deleted_node[list2[j]]:
                j += 1
            
            # If one of the lists is traversed, end the loop
            if i >= n or j >= m:
                break
    ...
    def is_dominated_edge(self,e):
        ...
        if is_dominated == False:
            return False
    def is_dominated_edge_set_relaxed(self,e, tolerance):
    ...
    def is_subset_with_deleted(self,sorted_list1, sorted_list2):
        ...
        while i < n:
            # Find next valid element in sorted_list1
            while i < n and self.deleted_node[sorted_list1[i]]:
                i += 1
                
            # Find next valid element in sorted_list2
            while j < m and self.deleted_node[sorted_list2[j]]:
                j += 1
            
            if i >= n:  # list1 is traversed
                return True
            
            if j >= m:  # list2 is traversed, but list1 is not finished
                return False

            if sorted_list1[i] < sorted_list2[j]:
                return False  # current element in list1 is less than current element in list2
            elif sorted_list1[i] == sorted_list2[j]:
                i += 1  # Move list1 pointer
            j += 1  # Always move list2 pointer
    ...
    def is_subset_with_deleted_Tolerance(self, sorted_list1, sorted_list2,w,tolerance):
        ...
        i, j = 0, 0
        n, m = len(sorted_list1), len(sorted_list2)
        mismatch_count = 0  # Record number of mismatched elements
    ...
    def strong_collapse(self):
        ...
        while self.node_queue:
            ...
            # Group nodes in a single traversal
            for node in NG_v:
    ...
    def edge_collapse(self):
        ...
        while self.edge_queue:
            ...
            # if (e[0]==627 and e[1]==442) or (e[0]==442 and e[1]==627):
            #     print("here")
    ...
    def insert_dominated_edges_2(self):
        ...
        while vertex_queue: 
            ...
            other_node = np.array(other_node, dtype=np.int32)  # In case it's not a NumPy array yet
...
# Get labels  
            ...
            # Conditional matching: test_mask is False and label matches
    ...
    def heruistic_delete_edge(self):
        ...
        # Get all non-zero element coordinates (edges)
    ...
    # Find the element that eventually points to itself
        while map[elem] != elem:
            elem = map[elem]
        return elem
    ...
    def make_coarsened_graph_old(self):
        ...
        # Map non-deleted nodes to new node indices
        for i, v in enumerate(self.deleted_node):
            if v == False and self.keep_mask[i] == True:
                old_to_new[i] = index
                index += 1

        # Map deleted nodes to the eventually retained node indices
        for i, v in enumerate(self.deleted_node):
        ...
        # Compute node features for the new graph as the average of old node features
    ...
    def get_all_edges(self, inverse=False):
        ...
        if not inverse:
            ...
            # Include bidirectional edges: each edge (i, j) and (j, i)
            edges = list(zip(rows, cols))
    ...
    def compute_betti_numbers(self):
        filtered_adj_matrix = self.M[~self.deleted_node, :]  # Delete rows
    ...
    def visualize_coarsened_graph(self, edge_index, node_labels, save_name="coarsened_graph.png"):
        print("Saving coarsened graph visualization...")
        ...
        nx.draw(G, pos, node_color=colors, with_labels=True, 
                cmap=plt.cm.get_cmap('tab20'), node_size=200, font_size=6)
        
        plt.title(f"Coarsened Graph Visualization ({len(G.nodes())} nodes)")
        plt.savefig(save_name)
        plt.close()
    ...
    def run_algorithm_relaxed_strong_collapse(self):
        ...
        ori_num_edges = len(self.remain_edges)
        index = 0
        ...
        #self.visualize_coarsened_graph(np.array(list(self.remain_edges)).T, self.label, save_name=f"relaxed_strong_collapse_before_hetero_{self.dataname}_{self.reduction_ratio:.2f}.png") 
        ...
        num_delete_hetero = ori_num_edges*self.args.del_edge_ratio
        self.delete_heterophlic_edge(num_delete_hetero )
        if self.save == True:    
...
if __name__ == "__main__":
    ...
    para_dict = {'Cora':{0.5:[15,15,0],0.3:[15,15,0],0.2:[15,15,0],0.1:[15,15,0]},
                "Citeseer":{0.5:[15,15,1000],0.3:[15,15,2000],0.2:[15,15,700],0.1:[15,15,2000]},
                    'dblp':{0.5:[10,10,0],0.3:[10,10,0],0.2:[10,10,0],0.1:[25,25,2000]},
    ...
    parser.add_argument('--deg1', type=int, default=35)
    parser.add_argument('--deg2', type=int, default=35)
    parser.add_argument('--del_edge', type=int, default=0)
    parser.add_argument('--del_edge_ratio', type=float, default=0.1)
    ...
    print("dataname {}, ratio {}, deg1 {}, deg2 {}".format(args.dataname,args.ratio,para_dict[args.dataname][args.ratio][0],para_dict[args.dataname][args.ratio][1]))
    user = "xxx" #replace with your username
...
# args.ratio = ratio
    ...
    #collapse.visualize_coarsened_graph(collapse.data['new_edge_index'], collapse.data['node_label'], save_name=f'coarsened_graph_{dataset_name}_{args.ratio}.png')
    time_end = time.time()
